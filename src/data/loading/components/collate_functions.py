...
def collate_fn_inference_for_sequence(
    ...
    **kwargs,
    ...
    for field_name, field_sequence in batch.items():  # type: ignore
        ...
        is_position_ids = field_name.endswith("_position_ids")
        field_padding_token = -1 if is_position_ids else padding_token

        # 1. in-batch padding s.t. all sequences have the same length and in the format of pt tensor
        current_sequence = pad_sequence(
            current_sequence, batch_first=True, padding_value=field_padding_token
        )
        ...
        current_sequence = pad_or_trim_sequence(
            ...
            sequence_length=sequence_length,
            padding_token=field_padding_token,
        )
        model_input_data.transformed_sequences[field_name] = current_sequence

        if (
            field_name not in id_field_name
            and model_input_data.mask is None
            and not is_position_ids
        ):
...
def collate_fn_train(
    ...
    variable_length_position_field: Optional[str] = None,
) -> Tuple[SequentialModelInputData, SequentialModuleLabelData]:
    ...
    if variable_length_position_field and variable_length_position_field in batch:
        position_sequence = pad_sequence(
            batch[variable_length_position_field],
            batch_first=True,
            padding_value=-1,
        )
        position_sequence = pad_or_trim_sequence(
            padded_sequence=position_sequence,
            sequence_length=sequence_length,
            padding_token=-1,
        )
    ...
    for field_name, field_sequence in batch.items():  # type: ignore
        ...
        if field_name in labels:
            ...
            if position_sequence is not None and hasattr(
                label_function, "position_sequence_key"
            ):
                label_kwargs["position_sequence"] = position_sequence
        ...
        if model_input_data.mask is None and not is_position_ids:
            mask_source = (
                label_function_output.sequence
                if field_name in labels
                else current_sequence
            )
            model_input_data.mask = (mask_source != padding_token).long()
